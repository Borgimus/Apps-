...
| P6-direct | supervisor (self) | fallback clause invoked; broker execution + order authority (the loop places paper orders) is the highest-stakes area — kept under supervisor | `src/live/*`, `src/broker/alpaca_client.py`, `src/data/alpaca_data.py`, `scripts/run_swing.py`, docs, 6 test files | `pytest -c pytest_swing.ini` (229 passed/1 skipped lean; 230 with SQLAlchemy) + entrypoint import + compile, run by supervisor | N/A (not delegated) | completed directly; mode-gating (SHADOW submits nothing), fail-closed entries, always-manage, and paper-endpoint enforcement verified | (fresh branch off merged main) |
| P6b-direct | supervisor (self) | fallback clause invoked; durable position/5R-once state is safety-critical (a restart must not double-take the partial) — kept under supervisor | `src/live/trade_state.py`, orchestrator store hooks, `scripts/run_swing.py`, docs, 2 test files | `pytest -c pytest_swing.ini` (238 passed/1 skipped lean; 239 with SQLAlchemy) + restart-safe 5R-once and broker-wins reconstruction verified by supervisor | N/A (not delegated) | completed directly; store persistence, restart 5R-once, and no-fabricated-position reconstruction verified | (PR #20 branch) |
