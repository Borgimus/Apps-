...
async def build_daily_report(db_session, session_date: str, settings=None) -> DailyReport:
    ...
    try:
        ...
        if bridge_rows:
            ...
            # Signals generated by the session runner are persisted in the
            # bridge table. Older reports queried only DBSignal, which is why
            # active sessions incorrectly displayed total_signals=0.
            report.total_signals = max(report.total_signals, len(bridge_rows))
            ...
            for _sid, _count in _bridge_signal_counts.items():
                ...
                _stats_by_strategy[_sid].signals = max(
                    _stats_by_strategy[_sid].signals,
                    _count,
                )
